...
public:
    ...
    const T& operator [] (const int& index) const { return data[index]; }  // ← 追加
public:
...
public:
    constexpr int get_size() const;  // ← constを追加
    constexpr int get_capacity() const;  // ← constを追加
public:
    T* iter_begin();
    T* iter_end();
};
...
template <typename T, int N>
constexpr int avec<T, N>::get_size() const  // ← constを追加
{
    return this->size;
}

template <typename T, int N>
constexpr int avec<T, N>::get_capacity() const  // ← constを追加
{
    return N;
}
