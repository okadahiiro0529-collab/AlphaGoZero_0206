...
int main(int argc, char** argv)
{
    ...
    // ===== データ収集器を初期化 ===== ← ここに追加
    data_exporter::DataExporter exporter("../../training_data/");
    // ================================
    ...
    while (logical < 100) {
        ...
        if (ai_result.candidates.empty()) {
            ...
            auto valid_moves = move::generate(field, tqueue[0].first == tqueue[0].second);
            // ===================
            ...
            // 3. 連鎖ボーナス（連鎖数に応じて増加）
            if (chain_len >= 2) {
                reward += chain_len * chain_len * 10.0f;  // 2連鎖=40, 3連鎖=90, 4連鎖=160
            }
            
            // 4. 即時連鎖スコアボーナス（この手で得たスコア）
            reward += chain_score * 0.2f;
            // ===================

            // ===== データ記録 =====
            exporter.record_step(
                field_before,       // 盤面
                valid_moves,        // avec<move:: Placement, 22>型
                mv.placement,       // 選択した手
                chain_score,        // 連鎖スコア
                chain_len,          // 連鎖数
                reward              // 報酬
    ...
    exporter.save_to_file(seed, score, max_chain_length, final_viewer);
    // ==================================
